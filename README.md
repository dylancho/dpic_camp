...
pip install -e ".[dev]"  # 원칙 d 채점기 (Python)
...
| `npm run adapt:d -- "<기업명>"` | 원칙 d 파이썬 채점기 실행 + 공용 형식 변환 (LLM 안 씀) |
| `npm run score -- "<기업명>"` | 채점만 (LLM 안 씀) |
| `npm test` | 채점·게이트 엔진 테스트 10개 |
| `python -m pytest` | 원칙 d 에이전트 테스트 145개 |
...
│   ※ 스코어링 전 필수. 안 하면 원칙마다 다른 잣대로 채점한다 │
└─────────────────────────────────────────────────────────────┘
      │
      ▼  같은 evidence.md + 같은 calibration.json 을 4개 원칙이 공유
┌──────────────┬──────────────┬──────────────┬───────────────┐
│  a           │  b           │  c           │  d            │
...
└──────────────┴──────────────┴──────────────┴───────────────┘
      ...
      ▼
┌─────────────────────────────────────────────────────────────┐
│ STEP 1-B · 원칙 d 파이썬 채점기      npm run adapt:d        │
...
각 원칙은 독립된 파일/모듈로 구현되어 하나의 파이프라인에 물려 있다.

| 원칙 | 배점 | 구현 위치 | 파이프라인에 붙는 방식 |
|---|---|---|---|
| a. Structural Demand | 30 | `.claude/skills/structural-demand-check/` (스킬) | `principle-a` 서브에이전트가 이 스킬을 루브릭으로 읽고 level로 변환 |
| b. Economic Value | 25 | `.claude/skills/economic-value-check/` (스킬) | `principle-b` 서브에이전트가 동일 방식 |
| c. Physical Impact | 20 | `.claude/agents/principle-c-physical-impact.md` | 2단계 서브에이전트 (하드 게이트 분리) |
| d. Proven Scalability | 25 | `agents/proven_scalability/` (Python 결정론 채점기) | `principle-d`가 **조사만** 하고, 채점은 파이썬이. `npm run adapt:d`가 변환 |
| 오케스트레이션 | — | `.claude/skills/투자심사/`, `lib/`, `scripts/` | 전체 조립 |

**원칙별 파일만 고치면 된다.** 서로 충돌하지 않는다.
...
| 새로 찾은 근거를 인용하면 "없는 sourceId"로 버려진다 | `extraEvidence` 필드를 계약서에 추가. 채점 직전에 근거 풀로 편입한다. d의 근거는 `psa-*` id로 편입 |
| level만 남으면 IC에서 설명이 안 된다 | `skillReport` 필드로 각 스킬이 정의한 표·서사 전문을 실어 보고서 본문에 그대로 쓴다 |
...
- **`app/` 웹 UI 버전도 남아 있다.** 배포용이 필요하면 `AI_GATEWAY_API_KEY`(유료 크레딧 필요)를
  넣고 `npm run dev`. 시연은 Claude Code 쪽이 비용 0원이라 더 편하다.
